...
public class UserService extends BaseUserService {
	...
	public Identity doFind(IdentityId identityId) {
        ...
        User localUser = User.find.where().eq("username", (identityId.userId())).findUnique();
        if(localUser == null) return null;
        SocialUser socialUser = new SocialUser(new IdentityId(localUser.username, localUser.provider), 
        	localUser.first_name, localUser.last_name, localUser.full_name,
            Option.apply(localUser.email), null, new AuthenticationMethod("userPassword"),
            null, null, Some.apply(new PasswordInfo("bcrypt", localUser.password_hash, Option.apply(localUser.password_salt)))
        );  
        if (Logger.isDebugEnabled()) {
